...
class LookupTestCase(BaseTestCase):
    ...
    def test_no_lookup_raises(self) -> None:
        ...
        with warnings.catch_warnings():
            ...
            for service, team in lookups:
                self.assertIsInstance(self.info.flag_ids(service, team), list)
                self.assertIsInstance(self.info.flag_ids(service, team), list)
                self.info.flag_ids_raw(service, team)
                self.info.flag_ids_raw(service, team)
    ...
    def test_unresolved_team_warns_instead_of_raising(self) -> None:
        ...
        with self.assertWarns(UserWarning) as caught:
            self.assertEqual([], self.info.flag_ids("ServiceA", self.info.team("typo")))
    ...
    def test_unknown_service_warns(self) -> None:
        with self.assertWarns(UserWarning) as caught:
            self.assertEqual([], self.info.flag_ids("nope", "nop"))
    ...
    def test_unknown_team_warns(self) -> None:
        with self.assertWarns(UserWarning) as caught:
            self.assertEqual([], self.info.flag_ids("ServiceA", "does-not-exist"))
        self.assertIn("Unknown team 'does-not-exist'", str(caught.warning))
        self.assertEqual(__file__, caught.filename)

    def test_invalid_team_type_warns(self) -> None:
        with self.assertWarns(UserWarning) as caught:
            self.assertEqual([], self.info.flag_ids("ServiceA", 4.0))  # type: ignore[arg-type]
    ...
    def test_known_team_without_flag_ids_stays_quiet(self) -> None:
        ...
        with warnings.catch_warnings():
            ...
            self.assertEqual([], info.flag_ids("ServiceA", "nop"))
            self.assertIsNone(info.flag_ids_raw("ServiceA", info.team("nop")))
    ...
    def test_resolvable_lookups_stay_quiet(self) -> None:
        with warnings.catch_warnings():
            ...
            for team in teams:
                self.assertEqual(["alice", "bob", "carol"], self.info.flag_ids("ServiceA", team))
...
class SelectRoundTestCase(BaseTestCase):
    ...
    def test_round_selects_one_round(self) -> None:
        self.assertEqual({"5": {"0": "alice", "1": "bob"}}, self.info.flag_ids_raw("ServiceA", "nop", 5))
        self.assertEqual(["alice", "bob"], self.info.flag_ids("ServiceA", "nop", 5))
        self.assertEqual(["carol"], self.info.flag_ids("ServiceA", "nop", 6))
        self.assertEqual(["alice", "bob"], self.info.flag_ids("ServiceA", "nop", 5))

    def test_negative_round_counts_back_from_the_newest(self) -> None:
        self.assertEqual(["carol"], self.info.flag_ids("ServiceA", "nop", -1))
        self.assertEqual(["alice", "bob"], self.info.flag_ids("ServiceA", "nop", -2))
    ...
    def test_default_is_every_published_round(self) -> None:
        ...
        self.assertEqual(["alice", "bob", "carol"], self.info.flag_ids("ServiceA", "nop"))
    ...
    def test_round_outside_the_published_window_is_empty(self) -> None:
        with warnings.catch_warnings():
            ...
            self.assertEqual([], self.info.flag_ids("ServiceA", "nop", 9999))
            self.assertEqual([], self.info.flag_ids("ServiceA", "nop", -99))
    ...
    def test_a_game_without_rounds_is_left_alone(self) -> None:
        ...
        team = faust.teams[0]
        every = faust.flag_ids(service, team)
        self.assertTrue(every)
        self.assertEqual(every, faust.flag_ids(service, team, 3))
